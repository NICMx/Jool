...
static void print_rfc6791v6_prefix(struct full_config *config, bool csv)
{
	...
	if (config->global.use_rfc6791_v6) {
		prefix = &config->global.rfc6791_prefix6;
		str = inet_ntop(AF_INET6, &prefix->addr, buffer,
				sizeof(buffer));
		if (str)
...
static int handle_display_response(struct jool_response *response, void *arg)
{
	...
	printf("  --%s: %s\n", OPTNAME_SRC_ICMP6E_BETTER,
			print_bool(conf->global.src_icmp6errs_better));
	printf("  --%s: %s\n", OPTNAME_HANDLE_FIN_RCV_RST,
			print_bool(conf->global.handle_rst_during_fin_rcv));

	printf("  --%s: %u (0b", OPTNAME_F_ARGS, conf->global.f_args);
	print_binary(conf->global.f_args, 4);
	printf(")\n");
	printf("    SrcAddr:%s", print_bool(conf->global.f_args & F_ARGS_SRC_ADDR));
	printf("  SrcPort:%s", print_bool(conf->global.f_args & F_ARGS_SRC_PORT));
	printf("  DstAddr:%s", print_bool(conf->global.f_args & F_ARGS_DST_ADDR));
	printf("  DstPort:%s\n", print_bool(conf->global.f_args & F_ARGS_DST_PORT));

	printf("  --%s: %s\n", OPTNAME_AMEND_UDP_CSUM,
			print_bool(conf->global.compute_udp_csum_zero));
	printf("  --%s: %u (%s)\n", OPTNAME_EAM_HAIRPIN_MODE,
			conf->global.eam_hairpin_mode,
			int_to_hairpin_mode(conf->global.eam_hairpin_mode));
	printf("  --%s: %s\n", OPTNAME_RANDOMIZE_RFC6791,
			print_bool(conf->global.randomize_error_addresses));
	...
	printf("    --%s: %s\n", OPTNAME_DROP_ICMP6_INFO,
			print_bool(conf->global.drop_icmp6_info));
...
static int handle_display_response_csv(struct jool_response *response, void *arg)
{
	...
	printf("%s,%s\n", OPTNAME_AMEND_UDP_CSUM,
			print_csv_bool(global->compute_udp_csum_zero));
	printf("%s,%s\n", OPTNAME_EAM_HAIRPIN_MODE,
			int_to_hairpin_mode(global->eam_hairpin_mode));
	printf("%s,%s\n", OPTNAME_RANDOMIZE_RFC6791,
			print_csv_bool(global->randomize_error_addresses));
	printf("%s,", OPTNAME_RFC6791V6_PREFIX);
	...
	printf("%s,%u\n", OPTNAME_MAX_SO,
			conf->bib.max_stored_pkts);
	printf("%s,%s\n", OPTNAME_SRC_ICMP6E_BETTER,
			print_csv_bool(global->src_icmp6errs_better));
	printf("%s,%u\n", OPTNAME_HANDLE_FIN_RCV_RST,
			global->handle_rst_during_fin_rcv);
	printf("%s,%u\n", OPTNAME_F_ARGS,
			global->f_args);
	printf("%s,%s\n", OPTNAME_BIB_LOGGING,
	...
	printf("%s,%s\n", OPTNAME_DROP_ICMP6_INFO,
			print_csv_bool(global->drop_icmp6_info));
	printf("%s,%s\n", OPTNAME_DROP_EXTERNAL_TCP,
