...
#ifdef __KERNEL__
#define GTYPE_FN(printer, packetizer, validator) .validate = validator
#define GLOBAL_FN(validator, printer) .validate = validator
#else
#define GTYPE_FN(printer, packetizer, validator) .print = printer, .packetize = packetizer
...
static struct global_type gt_bool = {
	.id = GTI_BOOL,
	.name = "Boolean",
	GTYPE_FN(print_bool, packetize_bool, NULL),
...
static struct global_type gt_uint8 = {
	...
	GTYPE_FN(print_u8, packetize_u8, validate_u8),
};

static struct global_type gt_uint32 = {
	.id = GTI_NUM32,
	.name = "32-bit unsigned integer",
	GTYPE_FN(print_u32, packetize_u32, validate_u32),
};

static struct global_type gt_timeout = {
	.id = GTI_TIMEOUT,
	.name = "[HH:[MM:]]SS[.mmm]",
	GTYPE_FN(print_timeout, packetize_timeout, validate_u32),
};

static struct global_type gt_plateaus = {
	.id = GTI_PLATEAUS,
	.name = "List of 16-bit unsigned integers separated by commas",
	GTYPE_FN(print_plateaus, packetize_plateaus, validate_plateaus),
};

static struct global_type gt_prefix6 = {
	.id = GTI_PREFIX6,
	.name = "IPv6 prefix",
	GTYPE_FN(print_prefix6, packetize_prefix6, validate_prefix6),
};

static struct global_type gt_prefix4 = {
	.id = GTI_PREFIX4,
	.name = "IPv4 prefix",
	GTYPE_FN(print_prefix4, packetize_prefix4, validate_prefix4),
};

static struct global_type gt_hairpin_mode = {
	.id = GTI_HAIRPIN_MODE,
	.name = "Hairpinning Mode",
	GTYPE_FN(print_hairpin_mode, packetize_hairpin_mode, validate_hairpin_mode),
