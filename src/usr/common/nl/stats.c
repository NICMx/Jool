...
static struct jstat_metadata const jstat_metadatas[] = {
	DEFINE_STAT(JSTAT_SUCCESS, "Successful translations.\n"
			"(Note: 'Successful translation' does not mean that the packet was actually delivered.)"),
	...
	DEFINE_STAT(JSTAT_HDR6, TC "Some IPv6 header field was bogus."),
	DEFINE_STAT(JSTAT_HDR4, TC "Some IPv4 header field was bogus."),
	DEFINE_STAT(JSTAT_UNKNOWN_L4_PROTO, TC "Packet carried an unknown transport protocol."),
	...
	DEFINE_STAT(JSTAT_DOUBLE_ICMP6_ERROR, TC "ICMPv6 error contained another ICMPv6 error."),
	DEFINE_STAT(JSTAT_DOUBLE_ICMP4_ERROR, TC "ICMPv4 error contained another ICMPv4 error."),
	DEFINE_STAT(JSTAT_UNKNOWN_PROTO_INNER, TC "ICMP error's inner packet had an unknown transport protocol."),
	DEFINE_STAT(JSTAT_HAIRPIN_LOOP, TC "Packet would have hairpin-looped infinitely."),
	DEFINE_STAT(JSTAT_POOL6_MISMATCH, TC "IPv6 packet's destination address did not match pool6."),
	DEFINE_STAT(JSTAT_POOL4_MISMATCH, TC "IPv4 packet's destination address and transport protocol did not match pool4."),
	...
	DEFINE_STAT(JSTAT_BIB_NOT_FOUND, TC "Packet was an ICMP error and did not match a BIB entry from the database."),
	...
	DEFINE_STAT(JSTAT_TYPE1PKT, "Total number of Type 1 packets stored. (See https://github.com/NICMx/Jool/blob/master/include/nat64/mod/stateful/bib/pkt_queue.h#L72)\n"
			"(This counter is not decremented when a packet leaves the queue.)"),
	DEFINE_STAT(JSTAT_TYPE2PKT, "Total number of Type 2 packets stored. (See https://github.com/NICMx/Jool/blob/master/include/nat64/mod/stateful/bib/pkt_queue.h#L72)\n"
			"(This counter is not decremented when a packet leaves the queue.)"),
	...
	DEFINE_STAT(JSTAT64_SRC, TC "IPv6 packet's source address did not match pool6 nor any EAMT entries, or the resulting address was blacklisted."),
	DEFINE_STAT(JSTAT64_DST, TC "IPv6 packet's destination address did not match pool6 nor any EAMT entries, or the resulting address was blacklisted."),
	...
	DEFINE_STAT(JSTAT64_ICMP_CSUM, TC "Incoming ICMPv6 packet's checksum was incorrect."),
	...
	DEFINE_STAT(JSTAT46_SRC, TC "IPv4 packet's source address was blacklisted, or did not match pool6 nor any EAMT entries."),
	DEFINE_STAT(JSTAT46_DST, TC "IPv4 packet's destination address was blacklisted, or did not match pool6 nor any EAMT entries."),
	...
	DEFINE_STAT(JSTAT46_ICMP_CSUM, TC "Incoming ICMPv4 packet's checksum was incorrect."),
	...
	DEFINE_STAT(JSTAT46_SRC_ROUTE, TC "Packet had an unexpired Source Route."),
	...
	DEFINE_STAT(JSTAT_FAILED_ROUTES, TC "The translated packet could not be routed."),
	...
	DEFINE_STAT(JSTAT_UNKNOWN, TC "Unknown errors."),
};
