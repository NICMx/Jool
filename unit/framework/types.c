		...
		unsigned char *local_addr, u16 local_id)
{
	...
	if (error) {
		log_err("Cannot parse '%s' as a valid IPv6 address", remote_addr);
		return error;
	}
	...
	if (error) {
		log_err("Cannot parse '%s' as a valid IPv6 address", local_addr);
		return error;
	}
		...
		unsigned char *local_addr, u16 local_id)
{
	...
	if (error) {
		log_err("Cannot parse '%s' as a valid IPv4 address", remote_addr);
		return error;
	}
	...
	if (error) {
		log_err("Cannot parse '%s' as a valid IPv4 address", local_addr);
		return error;
	}
