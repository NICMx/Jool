...
int set_updated_entries(void *data)
{
	...
	error = nl_send_simple(sender_sk, MSG_TYPE_JOOL, 0, received_data, received_data->length);
		if (error < 0) {
...
static int updated_entries_callback(struct nl_msg *msg, void *arg)
{
	...
	log_info("received header string -> %s",joold_data->magic);
...
/*	sender_callback = cb;
	...
	error = nl_socket_modify_cb(receiver_sk, NL_CB_VALID, NL_CB_CUSTOM, updated_entries_callback,NULL);
	...
	sockaddr.nl_pid = 1;
	sockaddr.nl_groups = JOOLD_MULTICAST_GROUP;
...
int get_updated_entries(void)
{
	...
	error = nl_recv(receiver_sk,&sockaddr,&received_data,NULL);
	printf("error code: %d\n",error);
	if (error < 0)
	printf("error: %s\n", nl_geterror(error));
