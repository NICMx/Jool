...
int main_wrapped(int argc, char **argv) {
	...
	if (strcmp(arguments.ip_version, "4")
			&& strcmp(arguments.ip_version, "6")) {
		log_err(
				"Inválid ip_version argument value, must be either \"6\" or \"4\" (without double quotes).\n");
		return -EINVAL;
	}
	...
	if (regexec(&regex, arguments.multicast_port, 0, NULL, 0)) {
		log_err(
				"Invalid multicast_port argument value, must be a valid integer!\n");
		return -EINVAL;
	}

	return init_threads(arguments.multicast_address, arguments.local_address,
			arguments.multicast_port);

	_error: argp_help(&argp_struct, stdout, ARGP_HELP_USAGE, "");
	fflush(stdout);
	return -1;
}
