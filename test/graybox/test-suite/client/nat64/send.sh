...
function test-ptb {
	# Create the test packet's session
	# (PTB packets are ICMP errors, which require a pre-existing session.)
	$GRAYBOX send `dirname $0`/manual/$1/session.pkt
	sleep 0.1

	# Actual test
	$GRAYBOX expect add `dirname $0`/manual/$1/receiver.pkt $2
	$GRAYBOX send `dirname $0`/manual/$1/sender.pkt
	sleep 0.1
	$GRAYBOX expect flush
}
...
if [[ -z $1 || $1 = *errors* ]]; then
	...
	test-manual icmp-error/ptb64-test icmp-error/ptb64-expected 1,2,3
	sudo ip netns exec joolns ip link set to_world_v4 mtu 1500

	sudo ip netns exec joolns ip link set to_world_v6 mtu 1280
	test-manual icmp-error/ptb46-test icmp-error/ptb46-expected 1,4,5,6,10,11
	sudo ip netns exec joolns ip link set to_world_v6 mtu 1500

	# TODO test every other ICMP error mandated by the RFC.
fi
...
#	# Same test, except hairpinning version.
#	# TODO same hairpinning problem as in ptb.
#	$GRAYBOX expect add `dirname $0`/manual/so/66-receiver.pkt 1,5,6,10,11
#	$GRAYBOX send `dirname $0`/manual/so/66-sender.pkt
#	sleep 8.1 # The timer runs every two seconds.
#	$GRAYBOX expect flush
