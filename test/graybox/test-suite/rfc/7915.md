...
Possible new local glossary entry:

- IP (Internal Packet): The chunk of data between the first ICMP header and the IE. This includes padding, because there's no way for Jool to tell the difference between internal packet bytes and padding.
...
The leftovers are old tests that don't fit in the new model, but there aren't any reasons to delete them.
...
3. OP quite truncated, IE preserved:
