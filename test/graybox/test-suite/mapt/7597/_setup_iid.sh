...
CE11_EUIP=2001:db8:4464:1::0100/128
CE11_ADDR4=192.0.2.1
CE11_ADDRL=192.168.0.1
CE12_EUIP=2001:db8:4464:1::0101/128
CE12_ADDR4=192.0.2.1
CE12_ADDRL=192.168.0.1
CE21_EUIP=2001:db8:4464:2::0100/128
...
add_nat() {
	...
	ip netns exec ${NAME}n iptables -t nat -A POSTROUTING \
			-s ${PRIVATE_ADDR} -o ${NAME}n_${NAME}t -p tcp \
			-j SNAT --to-source ${TO_SOURCE}
	ip netns exec ${NAME}n iptables -t nat -A PREROUTING \
			-i ${NAME}n_${NAME}t -p tcp --dport ${PRIVATE_PORT} \
			-j DNAT --to ${PRIVATE_ADDR}:${PRIVATE_PORT}
}

add_nat ce11 192.168.0.2 1024 192.0.2.1:1024-1027
add_nat ce11 192.168.0.3 2049 192.0.2.1:2048-2051
add_nat ce12 192.168.0.2 1030 192.0.2.1:1028-1031
add_nat ce21 192.168.0.2 1027 198.51.100.1:1024-1027
