...
static bool assert_deferred(struct joold_queue *joold, ...)
{
	...
	list_for_each_entry(actual, &joold->deferred.list, lh) {
		...
		if (!expected) {
			pr_err("Unexpected deferred session: " SEPP "\n",
					SEPA(&actual->session));
			success = false;
			goto end;
		}
	...
	expected = va_arg(args, struct session_entry *);
	if (expected != NULL) {
		pr_err("Session missing from deferred: " SEPP "\n", SEPA(expected));
		success = false;
		goto end;
	}
...
static bool assert_skb(int garbage, ...)
{
	...
	nla_for_each_nested(attr, root, rem) {
		error = jnla_get_session_joold(attr, "session", &cfg, &actual, NULL);
		if (error) {
			pr_err("jnla_get_session: errcode %d\n", error);
			success = false;
			goto end;
		}
		...
		expected = va_arg(args, struct session_entry *);
		if (!expected) {
			pr_err("Unexpected pkt session: " SEPP "\n", SEPA(&actual));
			success = false;
			goto end;
		}
	...
	expected = va_arg(args, struct session_entry *);
	if (expected != NULL) {
		pr_err("Session missing from packet: " SEPP "\n", SEPA(expected));
		success = false;
	}
